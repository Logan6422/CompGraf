...
std::vector<glm::vec2> generateTextureCoordinatesForSkyDome(const std::vector<glm::vec3> &positions) {
	...
	tex_coords.resize(positions.size()); // Tamano correcto (no funciona sino)
	
	for(int i = 0; i < positions.size(); i++) {
		float r = sqrt(pow(positions[i].x, 2) + pow(positions[i].y, 2) + pow(positions[i].z, 2));
		float tita = atan2(positions[i].z, positions[i].x); 
		float phi = acos(positions[i].y / r);
		
		// Normalizar las coordenadas de textura
		tex_coords[i].x = (tita + M_PI) / (2 * M_PI); 
		tex_coords[i].y = phi / M_PI; 
	}
