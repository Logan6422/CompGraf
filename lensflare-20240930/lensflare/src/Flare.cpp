...
std::vector<glm::vec2> Flare::generateTextureCoordinatesForFlare(float size, const glm::vec2& center) {
	std::vector<glm::vec2> texture_coords(4);
	...
	//difX = size -> difS = 1 (ancho y alto)
	//difX = -center.x (distancia a el borde izquierdo) --> difS = -center.x/size
	
	//difY = size -> difS = 1 (ancho y alto)
	//difY = -center.Y (distancia a el borde superior) --> difT = -center.y/size
	
	float halfsize = size*0.5f;
	
	
	float s0 = (-center.x + halfsize)/size; 
	float s1 = (win_width-(center.x - halfsize))/size;
	float t0 = (-center.y + halfsize)/size;
